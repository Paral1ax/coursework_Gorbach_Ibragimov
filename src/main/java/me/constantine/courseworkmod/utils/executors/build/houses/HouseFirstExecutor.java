...
public class HouseFirstExecutor {
    ...
    public static int cobbleCounter;
    public static int oakPlanksCounter;
    public static int glassCounter;
    ...
    private static void build() {
        ...
        if (!validationArea(start,end)){
            ...
            CourseWorkMod.MOB.sendMessage("AAA");
        }
            ...
            buildCeiling(start,min_x,min_y+4,min_z,max_x,max_z);
        }
    }
    ...
    private static void buildSecondLevel(Location start,double min_x,double min_y,double min_z,double max_x,double max_y,double max_z) {
        ...
        for (int i = 0; i < max_z - min_z -1; i++) {
            ...
            if (i==(max_z - min_z -2)/2-1||i==(max_z - min_z -2)/2)
                continue;
            start.getBlock().setType(Material.OAK_PLANKS,true);
        }
    ...
    private static void buildPiles(Location start,double min_x,double min_y,double min_z) {
        ...
        for (int i = 0; i < 4; i++) {
            start.add(0,i,0);
            start.getBlock().setType(Material.OAK_PLANKS);
            setDefaultLocation(start, min_x, min_y+1, min_z);
        }
    }
    ...
    private static void buildCeiling(Location start,double min_x,double min_y,double min_z,double max_x,double max_z){
        for (int i = 0; i < max_x - min_x+1; i++) {
            for (int j = 0; j < max_z - min_z+1; j++) {
                setDefaultLocation(start,min_x,min_y,min_z);
                start.add(i,0,j);
    ...
    private static boolean validationArea(Location start, Location end) {
        return Math.abs(end.getX() - start.getX()) < 4 || Math.abs(end.getY() - start.getY()) < 4;
    }
